...
let TabsMenu = {
  ...
  page: {
    ...
    onAttach: function(worker) {
      ...
      let fetch = () => {
        TabsMenu.fetchTabs(worker);
      };

      fetch();

      worker.port.on("fetch_tabs", fetch);

      worker.port.on("delete_all", function() {
        Application.tabController.deleteAll().then(fetch);
      });

      worker.port.on("delete_tab", function(data) {
        Application.tabController.deleteTab(data.id).then(fetch);
      });
      ...
      worker.port.on("delete_group", function(data) {
          Application.tabController.deleteGroup(data).then(fetch);
      });
      ...
      worker.on("detach", function() {
        TabsMenu.detachWorker(this);
      });
    },
  },
  detachWorker: function T_detachWorker(worker) {
    let index = TabsMenu.workers.indexOf(worker);
    if (index != -1) {
      TabsMenu.workers.splice(index,1);
    }
  },
  refreshWorkers: function T_refreshWorkers() {
    TabsMenu.workers.forEach(worker => {
      TabsMenu.fetchTabs(worker);
    });
  },
  fetchTabs: function T_fetchTabs(worker) {
    Application.tabController.getTabs().then(data => {
