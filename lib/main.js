  ...
  page: {
    ...
    onAttach: function(worker) {
      ...
      worker.port.on("delete_tab", function(evt) {
        Application.deleteTab(evt.url).then(fetchTabs);
      });
...
let Application = {
  ...
  deleteTab: function A_delete_tab(url) {
    let deletePromise = Promise.defer();
    let tabData = storage.tabs;
    ...
    deletePromise.resolve(delCount);
  ...
  openTabsMenu: function A_openTabsMenu() {
    ...
    // is this window new or old?
    /*
    Application.TabStoragePromise.then(function(tabStorage) {
      return tabStorage.createSession()
    });
    */
  ...
  init: function A_init() {
    ...
    console.log("init almost complete");
