...
TabStorage.prototype = {
  ...
  createTab: function TS_createTab(url, title, session_id) {
    return Task.spawn(function createTabTask() {
      ...
      let baseDomain;
      try {
        baseDomain = eTLDService.getBaseDomain(uri);
      }
      catch(error) {
        if (error.result == Cr.NS_ERROR_INSUFFICIENT_DOMAIN_LEVELS) {
          // URL doesn't look to have a base domain
          baseDomain = "special_hostname";
        }
        else if (error.result == Cr.NS_ERROR_HOST_IS_IP_ADDRESS) {
          baseDomain = "ip_address";
        }
        else {
          // empty URL, invalid characters and other undesirables
          console.error(error);
          return;
        }
      }
